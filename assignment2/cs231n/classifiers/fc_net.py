...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        hidden_layer = np.dot(X_row, W1) + b1
        ...
        dW1 = np.dot(X_row.T, dhidden_layer)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            if i == 0:
                self.params['W{}'.format(i+1)] = weight_scale * np.random.randn(input_dim, hidden_dims[i])
                ...
                self.params['W{}'.format(i+1)] = weight_scale * np.random.randn(hidden_dims[i-1], self.num_classes)
                self.params['b{}'.format(i+1)] = np.zeros(self.num_classes)
            else:
